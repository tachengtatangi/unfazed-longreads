...
def get_allele_at(read, pos, var_len):
    ...
    if pos in read_ref_positions:
        ...
        if len(read.query_sequence) > read_pos + var_len:
            base = read.query_sequence[read_pos: read_pos + var_len]
            return base
...
def snv_match_alleles(informative_reads, read, ref, alt, position):
    """
    ...
    read_allele = get_allele_at(read, position, variant_len)
...
def collect_reads_snv(
    ...
    try:
        bam_iter = bamfile.fetch(region["chrom"], position, position + 1)
    except ValueError:
        chrom = (
            region["chrom"].strip("chr")
            if "chr" in region["chrom"]
    ...
    for read in bam_iter:
        ...
        if len(ref) == len(alt):
            snv_match_alleles(informative_reads, read, ref, alt, position)
        # else:
    ...
    # return informative_reads, concordant_upper_len
    return informative_reads, bamfile
