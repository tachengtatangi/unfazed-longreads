...
def write_final_output(output_file):
    path = os.getcwd()
    with open("unfazed.txt") as f1:
        unfazed = f1.readlines()
    title = unfazed.pop(0)
    unfazed_starts = [item.split('\t')[0] + '\t' + item.split('\t')[1] for item in unfazed]

    with open("check.log") as f2:
        check = f2.readlines()
    check.pop(0)
    for line in check:
        check_start = line.split('\t')[0] + '\t' + line.split('\t')[1]
        check_origin = line.split('\t')[5]
        if check_start in unfazed_starts:
            for denovo in unfazed:
                if denovo.startswith(check_start):
                    unfazed_origin = denovo.split('\t')[5]
                    idx = unfazed.index(denovo)
                    break
            if check_origin == unfazed_origin:
                continue
            elif check_origin == "ambiguous":
                continue
            else:
                unfazed[idx] = line
        else:
    ...
    unfazed = sorted(unfazed, key=lambda x: (x.split('\t')[0], int(x.split('\t')[1])))
    with open(output_file, 'w') as w:
        w.write(title)
        w.writelines(unfazed)
    os.remove("unfazed.txt")
    os.remove("check.log")
...
def unfazed(args):
    ...
    write_final_output(args.outfile)
