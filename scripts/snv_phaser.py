...
from .site_searcher import verify
...
def get_refalt(chrom, pos, vcf_filehandle, kid_idx):
    ...
    # )
    region = "{}:{}-{}".format(chrom, pos + 1, pos + 1)
...
def multithread_read_phasing(
    ...
    dnm_reads, bam = collect_reads_snv(
        denovo["bam"],
        region,
        denovo["het_sites"],
        ref,
    ...
    result = verify(region["chrom"], dad_id, mom_id, bam, dnm_reads, informative_sites)
    front_half_output = [denovo['chrom'], str(denovo['start']), str(denovo['end']), denovo['vartype'], denovo['kid']]
    if result:
        output = front_half_output + result
        verify_list.append(output)
...
def run_read_phasing(
    ...
    if threads != 1:
        wait(futures)
    verification = sorted(verification, key=lambda x: (x[0], int(x[1])))
    verification = ['\t'.join(item) for item in verification]
    with open('check.log', 'w') as w:
        w.write('#chrom' + '\t' + 'start' + '\t' + 'end' + '\t' + 'vartype' + '\t' + 'kid' + '\t' + 'origin_parent' +
                '\t' + 'other_parent' + '\t' + 'evidence_count' + '\t' + 'evidence_types' + '\t' + 'origin_parent_sites'
                + '\t' + 'origin_parent_reads' + '\t' + 'other_parent_sites' + '\t' + 'other_parent_reads' + '\n')
